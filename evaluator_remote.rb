...
cores=ARGV[1]||1
id=JSON.parse(`curl -H "Content-Type: application/json" -d '{"fen": "#{fen}"}' -X POST http://www.chessanalysis.org/api/evaluations`)["id"]
slave=fork do
	...
	loop do
		...
		if data[3]=="seldepth"
			if update_count%5==0
				...
				`curl -H "Content-Type: application/json" -d '{"engine": "#{engine}","nodes":"#{data[9]}","seldepth":"#{data[4]}","evaluation": "#{data[7]}","time":"#{data[13]}"}' -X PUT http://www.chessanalysis.org/api/evaluations/"#{id}"`
				else
					#puts "{'depth':#{data[2]},'evaluation':#{data[7]},'nodes':#{data[10]},'seldepth':#{data[4]},'time':#{data[14]}}"
				`curl -H "Content-Type: application/json" -d '{"engine": "#{engine}","seldepth":"#{data[4]}","nodes":"#{data[10]}","evaluation": "#{data[7]}","time":"#{data[14]}"}' -X PUT http://www.chessanalysis.org/api/evaluations/"#{id}"`
				end			
			end
			update_count+=1

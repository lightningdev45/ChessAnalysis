			...
			loop do
				...
				if data[3]=="seldepth"
					if update_count%5==0
						...
						`curl -H "Content-Type: application/json" -d '{"depth":"#{data[2]}",engine": "#{engine}","nodes":"#{data[9]}","seldepth":"#{data[4]}","evaluation": "#{data[7]}","time":"#{data[13]}"}' -X PUT http://localhost:3000/evaluations/"#{id}"`
						...
						`curl -H "Content-Type: application/json" -d '{"depth":"#{data[2]}","engine": "#{engine}","seldepth":"#{data[4]}","nodes":"#{data[10]}","evaluation": "#{data[7]}","time":"#{data[14]}"}' -X PUT http://localhost:3000/evaluations/"#{id}"`
...
#end
sleep 1
fen=`curl -X GET http://localhost:4000/get_fen/#{connection_id}`
puts fen
cores=ARGV[1]||1
id=JSON.parse(`curl -H "Content-Type: application/json" -d '{"fen": "#{fen}"}' -X POST http://localhost:3000/evaluations`)["id"]
slave=make_slave(fen,cores,id)

listener=fork do
	
	loop do 
		temp_fen=`curl -X GET http://localhost:4000/get_fen/#{connection_id}`
		puts temp_fen
		if temp_fen=="kill"
